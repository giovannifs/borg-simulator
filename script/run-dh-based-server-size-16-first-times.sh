...
for i in `seq 0 15`; do
    ...
    sed -i -e 's/\$max_mem_capacity\$/16/g' server-size-simulation.conf
    sed -i -e 's/\$max_cpu_capacity\$/16/g' server-size-simulation.conf

    sed -i -e 's/\$workload_path\$/timestamps\/all-tasks-time-'"$i"'-for-java.csv/g' server-size-simulation.conf

    # command
    java -cp borg-simulator.jar org.cloudish.dh.DHMainExecutor server-size-simulation.conf > log-ss-1-on-min-1-time"$i".out 2> log-ss-1-on-min-1-time"$i".err
#    cat server-size-simulation.conf
    ...
    # command
    java -cp borg-simulator.jar org.cloudish.dh.DHMainExecutor server-size-simulation.conf > log-ss-1-on-min-12477-time"$i".out 2> log-ss-1-on-min-12477-time"$i".err
#    cat server-size-simulation.conf
    ...
    # command
    java -cp borg-simulator.jar org.cloudish.dh.DHMainExecutor server-size-simulation.conf > log-ss-1-off-min-1-time"$i".out 2> log-ss-1-off-min-1-time"$i".err
#    cat server-size-simulation.conf
    ...
    # command
    java -cp borg-simulator.jar org.cloudish.dh.DHMainExecutor server-size-simulation.conf > log-ss-1-off-min-12477-time"$i".out 2> log-ss-1-off-min-12477-time"$i".err
#    cat server-size-simulation.conf
